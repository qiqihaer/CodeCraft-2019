    ...
    for(size_t k=0;k<4;k++){
        if(cur_road_id[inc_index[k]]>=0){
            ...
            if(r.getS_id()==cross_index){
                if(r.getIs_two_way()){
                    try{scheduleWaitCars(cross_index,road_index,r.car_on_road_r,true);}
                    catch(int ID) {
                        throw ID;
                    }
                }
            }else if(r.getE_id()==cross_index){
                try{scheduleWaitCars(cross_index,road_index,r.car_on_road,false);}
                catch(int ID) {
                    throw ID;
                }
            }
...
void Scheduler::scheduleCars(vector<int>& start_cars, vector<int>& prior_start_cars, int ct){
    ...
    while(!all_car_end){
        ...
        for(size_t i=0;i<cross_id_inc.size();i++){
            try{scheduleCarsOnCross(cross_id_inc[i]);}
            catch(int ID) {
                throw ID;
            }
        }
...
int Scheduler::scheduleAllCars(vector<Answer>& answer_ve){
    ...
    while(time_consume<10000&&car_finish<answer_ve.size()){
        ...
        cout<<endl; */
        try{scheduleCars(start_cars,prior_start_cars,time_consume+1);}
        catch(int ID) {
            throw ID;
        }
        start_cars.clear();
